...
for /f "tokens=1,2,3,4 delims=," %%a in (complete_results.csv) do SET /A z= %%z &SET /A a= %%a & set /A b=%%b& set /A c=%%c
for /l %%p in (24, 24, 168) do (
   for /l %%l in (0,1,10) do (
       rem for /l %%d in (0,1,4) do (
              ...
              python prognose.py -p=%%p -lr=%%l  -cp=True -dp=True
