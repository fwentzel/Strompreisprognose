...
residual_prediction = ResidualPrediction(datacolumn="Residual", train_data=train_data, test_data=test_data, future_target=future_target, past_history=past_history, start_index_from_max_length=start_index_from_max_length)


for i in range(4) :
    for j in np.arange(0,0.6,0.1):
        train=True
        if train:
            pred.initialize_network(dropout=j,layers=i)
        ...
        train=True
        if train:
            residual_prediction.initialize_network(dropout=j,layers=i)
            residual_prediction.train_network(savename="trainedLSTM_resid2")
        else:
            residual_prediction.load_model(savename="trainedLSTM_resid2")
        ...
        print("layers:",i,"; dropout:",j,"; Residual Error:",res_error,"; complete Error:",complete_error)
